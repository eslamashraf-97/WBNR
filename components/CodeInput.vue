<template>
  ...
  >
    <div
      class="code-input flex flex-row-reverse gap-2 md:gap-3 lg:gap-4 xl:gap-5"
    >
      <template v-for="(v, index) in values" :key="index">
        <input
          class="text-center w-[30px] h-[50px] md:w-[40px] md:h-[60px] xl:w-[50px] xl:h-[70px] flex-1 border border-solid border-strk-75 dark:border-dark-textGray1 text-text-50 dark:text-dark-white rounded-xl text-xl xl:text-2xl"
          :class="{ 'border-primary-50 dark:border-dark-white': v }"
          type="text"
          placeholder="0"
          pattern="[0-9]"
