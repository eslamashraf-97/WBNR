...
import { a as apiGetCartUrl, f as apiGetGov, h as apiPlaceOrderUrl, i as apiAddDestinationUrl } from './index-f05587d3.mjs';
...
const _sfc_main = {
  ...
  async setup(__props) {
    ...
    const { data: cartData, pending } = ([__temp, __restore] = withAsyncContext(() => fireCart()), __temp = await __temp, __restore(), __temp);
    const { fire: fireGov } = useApi({
      url: apiGetGov,
      requestOptions: {
        query: {
          country_id: (_b = selectedCountry.value) == null ? void 0 : _b.id,
          take: 100
        }
      }
    });
    ...
    function chooseGov(data) {
      ...
      fireAddDestination({
        cart_id: (_a2 = cartData.value) == null ? void 0 : _a2.data.id,
        governorate_id: data.id
      });
    }
    const { fire: firePlaceOrder } = useApi({
      url: apiPlaceOrderUrl + "/" + ((_c = cartData.value) == null ? void 0 : _c.data.id),
      requestOptions: {
        method: "post"
      }
    });
          ...
          if (_push2) {
            ...
            return [
              createVNode("div", { class: "flex items-center gap-[19px]" }, [
                ...
                createTextVNode(" " + toDisplayString(((_b2 = unref(getSelectedGov)) == null ? void 0 : _b2.name) || "\u0627\u0644\u0645\u062D\u0627\u0641\u0638\u0629"), 1)
              ])
            ];
          }
        }),
...
//# sourceMappingURL=checkout-6398c554.mjs.map
