      ...
      <div class="p-4 mb-4">
        <div>
          <table class="w-full">
            ...
            <tbody>
              <tr v-for="(order, key) in data.data" :key="order.id" :ref="el => { divs[key] = el }">
                <td>
                  <img
                ...
                <td class="text-2xl text-gray-700 font-normal flex justify-center items-center leading-normal cursor-pointer"
                    @click=toggleRow(key)
                >
                  <svg :class="['rotate-180',{'rotate-0' : selected == key }]" xmlns="http://www.w3.org/2000/svg" width="22" height="12" viewBox="0 0 22 12" fill="none">
                    <path d="M20.8196 10.6718L11.0782 1.50042L1.02262 10.3263" stroke="#76757A" stroke-width="2" stroke-linecap="round"/>
                  </svg>
                </td>
                ...
                <td class='expanded-row-content hide-row px-4'>
                  <div class="timeline flex justify-between relative">
                    <div class="bg-white z-30">
                      ...
                      <p class="text-gray-200">24/09/2012</p>
                      <p  class="text-gray-400 text-xl font-semibold cursor-pointer"> لا يوجد تعليق</p>
                    </div>
                    <div class="bg-white z-30">
                      <svg width="98" height="98" viewBox="0 0 98 98" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <rect x="6.5" y="6.5" width="85" height="85" rx="42.5" fill="#E9F7F1"/>
                        <rect x="6.5" y="6.5" width="85" height="85" rx="42.5" stroke="white" stroke-width="13"/>
                        <path fill-rule="evenodd" clip-rule="evenodd" d="M43.0939 58.6543C43.1057 60.3062 42.1445 61.802 40.6599 62.4427C39.1752 63.0832 37.4605 62.7417 36.3175 61.5778C35.1744 60.4142 34.8289 58.658 35.4425 57.1307C36.0561 55.6032 37.5076 54.6065 39.1182 54.6065C40.1688 54.6026 41.1778 55.0267 41.9234 55.7858C42.669 56.5449 43.09 57.5768 43.0939 58.6543Z" stroke="#05BD6E" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                        <path fill-rule="evenodd" clip-rule="evenodd" d="M62.0363 58.6543C62.048 60.3062 61.0869 61.802 59.6022 62.4427C58.1177 63.0832 56.4029 62.7417 55.2599 61.5778C54.1168 60.4142 53.7713 58.658 54.3849 57.1307C54.9984 55.6032 56.45 54.6065 58.0605 54.6065C59.1112 54.6026 60.1202 55.0267 60.8657 55.7858C61.6114 56.5449 62.0324 57.5768 62.0363 58.6543Z" stroke="#05BD6E" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                    ...
                    <div class="bg-white z-30">
                      <svg width="98" height="98" viewBox="0 0 98 98" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <rect x="6.5" y="6.5" width="85" height="85" rx="42.5" fill="#E9F7F1"/>
                        <rect x="6.5" y="6.5" width="85" height="85" rx="42.5" stroke="white" stroke-width="13"/>
                        <path d="M56.0818 34.8389C59.9323 34.8603 62.0176 35.0311 63.378 36.3914C64.9336 37.9469 64.9336 40.4506 64.9336 45.4578V56.08C64.9336 61.0872 64.9336 63.5908 63.378 65.1464C61.8224 66.702 59.3187 66.702 54.3115 66.702H43.6894C38.6822 66.702 36.1785 66.702 34.6229 65.1464C33.0674 63.5908 33.0674 61.0872 33.0674 56.08V45.4578C33.0674 40.4506 33.0674 37.9469 34.6229 36.3914C35.9832 35.0311 38.0685 34.8603 41.9191 34.8389" stroke="#05BD6E" stroke-width="2.8"/>
                        <path d="M43.6895 51.4798L46.7244 54.3123L54.3115 47.231" stroke="#05BD6E" stroke-width="2.8" stroke-linecap="round" stroke-linejoin="round"/>
                    ...
                    <!-- Lines -->
                    <div class="sm:flex w-full top-[28%] z-1 absolute bg-primary-100 h-1"></div>
                    <div :class="['sm:flex left-0 top-[28%] z-20 absolute bg-primary-300 h-[0.28rem]', calculateWidthOfDone(order.status), {'bg-red-900' : order.status == 'cancelled'}]"></div>
                    <div :class="['sm:flex border border-dashed border-primary-300 border-[2.8px] left-0 top-[28%] z-10 absolute', calculateWidthOfWait(order.status)]"></div>
                  </div>
                  ...
                  <div class="my-8 px-6 max-h-72 overflow-auto">
                    ...
                    >
                      <div class="flex justify-between w-full items-center">
                        ...
                        <div>{{product.status_if_returned_before}}</div>
                        <button
                            class="ms-auto flex items-center gap-4 text-primary-300 disabled:text-primary-200 text-lg"
                            @click="showReturnModal({...product, order: {id: order.id}})"
                  ...
                  <div class="px-6 flex gap-28">
                    <div class="flex-1 text-start">
                      <div class="mb-6 flex justify-between items-center">
                        ...
                        <p class="m-0 text-gray-500 text-xl">{{ order.order_number }}</p>
                      </div>
                      <div class="mb-6 flex justify-between items-center">
                        <p class="m-0 text-gray-700 text-2xl">اسم العميل:</p>
                        <p class="m-0 text-gray-500 text-xl">{{ order.client.name }}</p>
                      </div>
                      <div class="mb-6 flex justify-between items-center">
                        <p class="m-0 text-gray-700 text-2xl">تاريخ الطلب:</p>
                        <p class="m-0 text-gray-500 text-xl">{{ handleDate(order.created_at) }}</p>
                      </div>
                      <div class="mb-6 flex justify-between items-center">
                        <p class="m-0 text-gray-700 text-2xl">المبلغ:</p>
                        <p class="m-0 text-gray-500 text-xl">{{ order.final_price }} {{ order.country.currency }}</p>
                      </div>
                    </div>
                    <div class="flex-1 text-start">
                      <div class="mb-6 flex justify-between items-center">
                      <p class="m-0 text-gray-700 text-2xl">سعر المنتجات:</p>
                      <p class="m-0 text-gray-500 text-xl">{{ order.price }} {{ order.country.currency }}</p>
                    </div>
                      <div class="mb-6 flex justify-between items-center">
                        <p class="m-0 text-gray-700 text-2xl">الربح:</p>
                        <p class="m-0 text-gray-500 text-xl">{{ order.profit }} {{ order.country.currency }}</p>
                      </div>
                      <div class="mb-6 flex justify-between items-center">
                        <p class="m-0 text-gray-700 text-2xl">الضريبة:</p>
                        <p class="m-0 text-gray-500 text-xl">{{ order.tax_amount }} {{ order.country.currency }}</p>
                      </div>
                      <div class="mb-6 flex justify-between items-center">
                        <p class="m-0 text-gray-700 text-2xl">سعر الشحن:</p>
                        <p class="m-0 text-gray-500 text-xl">{{ order.delivery_cost }} {{ order.country.currency }}</p>
                      </div>
                    </div>
                  </div>
                </td>
